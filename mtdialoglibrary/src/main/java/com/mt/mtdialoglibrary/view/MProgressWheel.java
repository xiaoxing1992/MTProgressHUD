...
public class MProgressWheel extends View {
  ...
  public MProgressWheel(Context context, AttributeSet attrs) {
    ...
    parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.MTProgressWheel));
  ...
  private void parseAttributes(TypedArray a) {
    ...
    circleRadius =
        (int) a.getDimension(R.styleable.MTProgressWheel_mt_circleRadius, circleRadius);

    fillRadius = a.getBoolean(R.styleable.MTProgressWheel_mt_fillRadius, false);

    barWidth = (int) a.getDimension(R.styleable.MTProgressWheel_mt_barWidth, barWidth);

    rimWidth = (int) a.getDimension(R.styleable.MTProgressWheel_mt_rimWidth, rimWidth);

    float baseSpinSpeed =
        a.getFloat(R.styleable.MTProgressWheel_mt_spinSpeed, spinSpeed / 360.0f);
    spinSpeed = baseSpinSpeed * 360;

    barSpinCycleTime =
        a.getInt(R.styleable.MTProgressWheel_mt_barSpinCycleTime, (int) barSpinCycleTime);

    barColor = a.getColor(R.styleable.MTProgressWheel_mt_barColor, barColor);

    rimColor = a.getColor(R.styleable.MTProgressWheel_mt_rimColor, rimColor);

    linearProgress = a.getBoolean(R.styleable.MTProgressWheel_mt_linearProgress, false);

    if (a.getBoolean(R.styleable.MTProgressWheel_mt_progressIndeterminate, false)) {
      spin();
    }
