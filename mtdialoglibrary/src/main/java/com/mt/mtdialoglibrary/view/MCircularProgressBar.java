...
public class MCircularProgressBar extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        rectF = new RectF();
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.MTCircularProgressBar, 0, 0);
        //Reading values from the XML layout
        try {
            // Value
            progress = typedArray.getFloat(R.styleable.MTCircularProgressBar_mt_progress, progress);
            // StrokeWidth
            strokeWidth = typedArray.getDimension(R.styleable.MTCircularProgressBar_mt_progressbar_width, strokeWidth);
            backgroundStrokeWidth = typedArray.getDimension(R.styleable.MTCircularProgressBar_mt_background_progressbar_width, backgroundStrokeWidth);
            // Color
            color = typedArray.getInt(R.styleable.MTCircularProgressBar_mt_progressbar_color, color);
            backgroundColor = typedArray.getInt(R.styleable.MTCircularProgressBar_mt_background_progressbar_color, backgroundColor);
        } finally {
            typedArray.recycle();
        }
